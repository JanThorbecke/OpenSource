...
/*int CoordinateCompare2dFloat(const void *a, const void *b){
    int x1=*(float*)a;
    int x2=*(float*)b;
    if(x1>x2) return( 1);
    if(x1<x2) return(-1);
...
*/
	...
	for(i=0,tn=0;i<ntr;i++){
		...
		if(count!=TRCBYTES){
			...
			if(hdr.scalel<0) scalel=1/((float)-hdr.scalel);else scalel=(float)hdr.scalel;
			if(hdr.scalco<0) scalco=1/((float)-hdr.scalco);else scalco=(float)hdr.scalco;
			...
			if(x[tn]<mod->origx||x[tn]>mod->xmax||z[tn]<mod->origz||z[tn]>mod->zmax){
				if(verbose>1){
					...
					vwarn("%f [xmin]<= %f [xr] <=%f xmax",mod->origx,xi[tn],mod->xmax);
					vwarn("%f [zmin]<= %f [zr] <=%f zmax",mod->origz,zi[tn],mod->zmax);
				}
				continue;
			}
