...
int readDataTD(char *filename, float xmin, float dx, float *xrcv, float *xsrc, int *xnx, float *rdata, int nw, int nw_low, int ngath, int nx, int nxm, int ntfft, float alpha, float scale, float conjg, int transpose, int verbose)
{
	FILE *fp;
	segy hdr;
	size_t nread;
	...
	while (!end_of_file) {
		...
		for (i=0; i<itrace; i++) {
			...
			if (transpose == 0) {
        		for (iw=0; iw<ntfft; iw++) { // TIME - SHOTS - RECEIVERS
        			rdata[igath*ngath*ntfft+i*ntfft+iw] = scale*trace[i*ntfft+iw];
        			//rdata[iw*ngath*nx+igath*nx+i].i = conjg*scale*trace[nw_low+iw].i;
        		}
			}
			else {
        		for (iw=0; iw<ntfft; iw++) { // TIME - RECEIVERS - SHOTS
        			rdata[i*ntfft*nx+igath*ntfft+iw] = scale*trace[i*ntfft+iw];
        			//rdata[iw*ngath*nx+i*ngath+igath].i = conjg*scale*trace[nw_low+iw].i;
        		}
			}
		}
