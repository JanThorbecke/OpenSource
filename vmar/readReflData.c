...
int readReflData(char *filename, float xmin, float dx, float *xrcv, float *xsrc, int *xnx, complex *cdata, complex *cdata2, int nw, int nw_low, int ngath, int nx, int nxm, int ntfft, float alpha, float scale, float conjg, int transpose, int verbose)
{
	FILE *fp;
	segy hdr;
	size_t nread;
		...
		for (i=0; i<itrace; i++) {
			...
			if (transpose == 0) {
        		for (iw=0; iw<nw; iw++) { // FREQUENCY - SHOTS - RECEIVERS
					...
					cdata2[iw*ngath*nx+igath*nx+i].r = scale*ctrace[nw_low+iw].r;
        			cdata2[iw*ngath*nx+igath*nx+i].i = -conjg*scale*ctrace[nw_low+iw].i;
        		}
			}
			else {
        		for (iw=0; iw<nw; iw++) { // FREQUENCY - RECEIVERS - SHOTS
					...
					cdata2[iw*ngath*nx+i*ngath+igath].r = scale*ctrace[nw_low+iw].r;
        			cdata2[iw*ngath*nx+i*ngath+igath].i = -conjg*scale*ctrace[nw_low+iw].i;
        		}
			}
		}
