...
/**
*  Writes Decomposition Direction To Disk
*
*   AUTHOR:
*           Max Holicki
*
...
int writeDecompDirect(modPar *mod,float *dd){
	FILE *fp;
	segy hdr;
	size_t i;
	...
	/* Write Data To Disk */
	/**********************/
	fp=fopen(mod->file_dd,"w");
	fwrite(&hdr,1,TRCBYTES,fp);
	fwrite(dd,sizeof(float),mod->nz,fp);
	for(i=1;i<mod->nx;i++){
		/* Update Header */
		hdr.tracl++;
		hdr.tracf++;
		...
		/* Write to Disk */
		fwrite(&hdr,1,TRCBYTES,fp);
		fwrite(&(dd[i*mod->nz]),sizeof(float),mod->nz,fp);
	}
	fclose(fp);
