...
int writeRec(recPar rec, modPar mod, int ixsrc, int izsrc, int nsam, int ishot, int fileno, 
			 ...
			 float *rec_p, float *rec_pp, float *rec_ss, int verbose)
{
    ...
    int irec, ntfft, nfreq, nkx, xorig;
	int append;
	double ddt;
	...
	if (rec.type.ud && (mod.ischeme==1 || mod.ischeme==2) )  {
		...
		rec_up  = (float *)calloc(nsam*nkx,sizeof(float));
		rec_down= (float *)calloc(nsam*nkx,sizeof(float));
		...
		rec_vze = rec_up;
		rec_pe  = rec_down;
		/* copy input data into extended arrays and pad zeroes */
		memcpy(rec_vze,rec_vz,rec.n*nsam*sizeof(float));
		memcpy(rec_pe,rec_p,rec.n*nsam*sizeof(float));

		/* transform from t-x to kx-w */
		xorig = 0;
		xt2wkx(rec_vze, crec_vz, nsam, nkx, nsam, nkx, xorig);
		xt2wkx(rec_pe, crec_p, nsam, nkx, nsam, nkx, xorig);
		...
		wkx2xt(crec_up, rec_up, nsam, nkx, nkx, nsam, xorig);
		wkx2xt(crec_dw, rec_down, nsam, nkx, nkx, nsam, xorig);

		/* reduce array to ntout samples */
		free(crec_vz);
		free(crec_p);
		free(crec_up);
		free(crec_dw);
