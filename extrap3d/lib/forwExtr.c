...
void forwExtr(complex *oper, float k, float dx, float dz, int nkx)
{
	...
	for (ikx = 0; ikx <= (nkx/2); ikx++) {
		...
		kz2 = k2 - kx2;
		if (kz2 >= 0.0) {
			oper[ikx].r = cos(dz*sqrt(kz2));
			oper[ikx].i = sin(dz*sqrt(kz2));
		}
		else {
			oper[ikx].r = exp(-dz*sqrt(-kz2));
			oper[ikx].i = 0.0;
		}
	}
...
void forwExtr_ph(complex *oper, float k, float dx, float dz, float alfa, int nkx)
{
	...
	for (ikx = 1; ikx <= nkx/2; ikx++) {
		...
		oper[ikx].i = -1.0*sin(phase[ikx]);
	}
	oper[0].r = cos(phase[0]);
	oper[0].i = -1.0*sin(phase[0]);
