...
int main (int argc, char **argv)
{
    ...
    long    nw, nw_low, nw_high, nfreq, *xnx, *xnxsyn, istart, iend;
    long    reci, mode, n2out, n3out, verbose, ntfft, zfp;
    long    iter, niter, tracf, *muteW, *tsynW, ii, ampest, plane_wave, t0shift;
    ...
    double  t0, t1, t2, t3, t4, tii, tsyn, tread, tfft, tcopy, energyNi, *energyN0, tolerance;
    float   d1, d2, d3, f1, f2, f3, fxsb, fxse, fysb, fyse, ft, fx, fy, *xsyn, *ysyn, dxsrc, dysrc;
    float   *green, *f2p, *G_d, *DD, *RR, dt, dx, dy, dxs, dys, scl, mem;
...
/*================ Reading info about shot and initial operator sizes ================*/
    ...
    if(!getparlong("istart", &istart)) istart=20;
    if(!getparlong("iend", &iend)) iend=nt;
...
/*================ Allocating all data arrays ================*/
    ...
    DD      = (float *)calloc(Nfoc*nys*nxs*ntfft,sizeof(float));
    RR      = (float *)calloc(Nfoc*nys*nxs*ntfft,sizeof(float));
...
for (l = 0; l < Nfoc; l++) {
            for (k = 0; k < nys; k++) {
                for (i = 0; i < nxs; i++) {
                    for (j = 0; j < nts; j++) {
                        DD[l*nys*nxs*nts+k*nxs*nts+i*nts+j] = G_d[l*nys*nxs*nts+k*nxs*nts+i*nts+j];
                    }
                }   
            }   
        }   
for (l = 0; l < Nfoc; l++) {
            for (k = 0; k < nys; k++) {
                for (i = 0; i < nxs; i++) {
                    for (j = 0; j < nts; j++) {
                        G_d[l*nys*nxs*nts+k*nxs*nts+i*nts+j] = 0.0;
                    }
                }   
            }   
        }   
...
for (ii = istart; ii < iend; ii++) {
  ...
  for (l = 0; l < Nfoc; l++) {
            for (k = 0; k < nys; k++) {
                for (i = 0; i < nxs; i++) {
                    for (j = 0; j < nts; j++) {
                        G_d[l*nys*nxs*nts+k*nxs*nts+i*nts+j] = DD[l*nys*nxs*nts+k*nxs*nts+i*nts+j];
                    }
                }   
            }   
        }   
 ...
 for (l = 0; l < Nfoc; l++) {
                for (i = 0; i < npos; i++) {
                    ...
                    ix = ixpos[i]; 
                    iy = iypos[i]; 
                   f1min[l*nys*nxs*nts+i*nts+j] = -DD[l*nys*nxs*nts+iy*nxs*nts+ix*nts+j];
                    for (j = 1; j < nts; j++) {
                   f1min[l*nys*nxs*nts+i*nts+j] = -DD[l*nys*nxs*nts+iy*nxs*nts+ix*nts+nts-j];
                    }   
            }   
        }  
/*================ initialization ================*/
...
for (l = 0; l < Nfoc; l++) {
            for (k = 0; k < nys; k++) {
                for (i = 0; i < nxs; i++) {
                        RR[l*nys*nxs*nts+k*nxs*nts+i*nts+ii] = f1min[l*nys*nxs*nts+k*nxs*nts+i*nts+ii];
                }   
            }   
        }  
        tii = wallclock_time() - t4;
        if (verbose) vmess("*** time steps %li finished from %li time-steps ***", ii-istart, iend-istart);
        if (verbose) vmess("*** time step %li took %f seconds ***", ii, tii);
} /*end of MME */
    free(Ni);
...
/*================ write output files ================*/
    ...
    for (l = 0; l < Nfoc; l++) {
        ...
        if (file_f2 != NULL) {
            if (zfp==0) {
                ret = writeData3D(fp_f2, (float *)&RR[l*size], hdrs_out, n1, n2*n3);
                if (ret < 0 ) verr("error on writing output file.");
            }
            else {
                zfpcompress((float *)&RR[l*size],n2,n3,n1,tolerance,zfpm,fp_f2);
            }
        }
        if (file_f1plus != NULL) {
            if (zfp==0) {
