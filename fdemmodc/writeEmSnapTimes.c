...
int writeEmSnapTimes(modPar mod, snaPar sna, int ixsrc, int izsrc, int itime, float *vx, float *vz, float *tzz, float *txx, float *txz, int verbose)
{
	...
	if ( (((itime-sna.delay) % sna.skipdt)==0) && 
		...
		if (sna.type.vx)  fpvx  = fileOpen(sna.file_snap, "_shz", append);
		if (sna.type.vz)  fpvz  = fileOpen(sna.file_snap, "_shx", append);
		if (sna.type.p)   fpp   = fileOpen(sna.file_snap, "_sey", append);
